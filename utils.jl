...
@delay function hfun_paperswithtags(params)::String
	...
	io = IOBuffer()
	write(io, "<p style=\"margin-bottom: 10px;\">")
	for (i, path) in enumerate(paths)
		html = replace(path, joinpath("src", "pages") => "pub")
		html = replace(html, r".md$" => "")
...
"""
	hfun_recentnews()::String

Displays recent news posts (up to 4). Must be in `news` folder and have `news` tag.

Use with `{{recentblogposts}}`.

TODO: extend to take folder, tag as arguments
"""
function hfun_recentnews(params = nothing)::String
	folder = "news"
	tag = "news"
	if !isnothing(params)
		if length(params) == 2
	...
	for (i, path) in enumerate(paths)
		...
		if i >= 4
			# show "view all"
			write(io, "<b><a href=\"/tag/news/\">all posts</a></b><br>")
			break
		end
	end
	return String(take!(io))
